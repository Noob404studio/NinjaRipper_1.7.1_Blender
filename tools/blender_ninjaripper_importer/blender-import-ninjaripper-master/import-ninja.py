...
class BaseDuplicateTracker(object):
    ...
    def get_material(self, rip, texset):
        ...
        first = True
        for i, key in enumerate(sorted(texset.keys())):
            tex = texset[key]
            tex_obj = self.get_texture(os.path.join(rip.dirname, tex))
            
            if hasattr(tex_obj, 'image') and tex_obj.image:
                node_tex = nodes.new("ShaderNodeTexImage")
                node_tex.image = tex_obj.image
                node_tex.location = (-300, -300 * i)
                node_tex.label = f"Texture {key}"
                if first:
...
class RipConversion(object):
    ...
    def convert_object(self, rip, scene, obj_name):
        ...
        if not found:
            scene.collection.objects.link(nobj)
...
def register():
    ...
    # This places the option in the File > Import menu
